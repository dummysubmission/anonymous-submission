#!/usr/bin/env python3
"""Build OpenAI-compatible offline batch JSONL for Stage 1 intention bank generation."""
...
def main(cfg: DictConfig) -> None:
    ...
    # Resume: skip personas that already have bank entries in the output file.
    ...
    with batch_path.open("w", encoding="utf-8") as out_f:
        for sample in tqdm(dataset, total=len(dataset)):
            ...
            if not persona_id:
                print("Skipping persona sample with missing uuid.", flush=True)
                continue
    ...
    print(
        ...
        f"(bank_size={bank_size}, model={model}, save_path target={save_path})."
    )
